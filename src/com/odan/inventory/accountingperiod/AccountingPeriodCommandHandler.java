package com.odan.inventory.accountingperiod;

import java.sql.Timestamp;

import com.odan.finance.accountingperiod.*;
import com.odan.finance.accountingperiod.command.ReopenAccountingPeriod;
import com.odan.finance.accountingperiod.command.UpdateAccountingPeriod;
...
import com.odan.finance.accountingperiod.command.CreateAccountingPeriod;
import com.odan.finance.accountingperiod.command.DeleteAccountingPeriod;
...
public class AccountingPeriodCommandHandler implements ICommandHandler {
	...
	public void handle(CreateAccountingPeriod c) {
		...
		try {
			...
			if ((new com.odan.finance.accountingperiod.AccountingPeriodQueryHandler()).getByTimestamp(startDate, ownerId) != null
					|| (new com.odan.finance.accountingperiod.AccountingPeriodQueryHandler()).getByTimestamp(endDate, ownerId) != null) {
			...
			ap.setStatus(Flags.AccountingPeriodStatus.OPEN.getFlag());
	...
	public void handle(CloseAccountingPeriod c) {
		...
		try {
			Query q = new Query();
			q.set("id", c.get("id"));
			AccountingPeriod ap = (AccountingPeriod) (new com.odan.finance.accountingperiod.AccountingPeriodQueryHandler()).get(q);
			ap.setStatus(Flags.AccountingPeriodStatus.CLOSED.getFlag());
			HibernateUtils.save(ap, trx);
